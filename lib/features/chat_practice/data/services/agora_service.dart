...
class AgoraService {
  ...
  Future<void> initializeAndJoin({
    ...
    log("before joinChannel Agora ${_engine == null}");
    // ✅ Only joinChannel outside — because you might join different channels
    await _engine!.joinChannel(
      token: token ?? Secrets.agoraToken,
      channelId: channelId,
      uid: randomUid,
  ...
  Future<void> initializeAndUseWaitingRoom() async {
    int randomUid = math.Random().nextInt(1000000);
    await initializeAndJoin(channelId: "waiting_room", token: Secrets.agoraToken);
  }
}
